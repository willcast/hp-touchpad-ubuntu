...
{
	struct termios2 ti2;
	int bspeed = uart_speed(speed);
	printf("Tring to set speed %d\n", speed);
...
{
	int res,i,bytes=count;
	char* origbuf = (char*)buf;
	...
	} while (count && (count >= 0 || errno == 0 || errno == EINTR));
...
struct uart_t uart[] = {
	...
	{ "bcsp",       0x0000, 0x0000, HCI_UART_BCSP, 200, 200, FLOW_CTL,        NULL, bcsp     },
